...
class TorrentSearch extends Component
{
    ...
    final public function mount(Request $request): void
    {
        ...
        // PTT: domyślna ilość na stronę z preferencji użytkownika (cookie ptt_per_page ustawiane w profilu).
        // Czytamy z $_COOKIE, bo cookie jest ustawiany plaintextem po stronie klienta i EncryptCookies go pomija.
        if ($request->missing('perPage')) {
            $pttPerPage = (int) ($_COOKIE['ptt_per_page'] ?? 0);
            if ($pttPerPage > 0) {
                $this->perPage = min($pttPerPage, 100);
            }
        }
    }
