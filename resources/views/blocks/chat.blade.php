...
{{-- PTT: polling wiadomości gdy websocket (Echo) nie działa — nowe wiadomości
     pojawiają się bez odświeżania strony. Fallback, nieszkodliwy gdy WS działa. --}}
<script nonce="{{ HDVinnie\SecureHeaders\SecureHeaders::nonce('script') }}">
(function () {
    function startChatPolling() {
        var el = document.getElementById('chatbody');
        if (!el || !window.Alpine) { return setTimeout(startChatPolling, 1500); }
        setInterval(function () {
            try {
                var d = window.Alpine.$data(el);
                if (!d || !d.state || d.state.ui.loading) return;
                if (document.hidden) return;
                if (d.state.chat.room > 0 && d.state.chat.target < 1 && d.state.chat.bot < 1
                    && typeof d.fetchMessages === 'function') {
                    d.fetchMessages();
                }
            } catch (e) {}
        }, 4000);
    }
    if (document.readyState === 'complete') startChatPolling();
    else window.addEventListener('load', startChatPolling);
})();
</script>
